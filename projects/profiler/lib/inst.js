...
(function (isNode) {
    ...
    astgen = {
        shoes:function(statementName){return ESP.parse('triggerTimer("'+statementName+'");')},
        empty: { type: SYNTAX.Identifier.name, name: '' },
...
var base=(function(){
    ...
    var lastTimeHashId = 0;

    function triggerTimer(id) {
        time=performance.now();
        if (timeHash[id] === undefined) {
            hitHash[id] = 0;
            timeHash[id] = 0;
        }
        hitHash[id] ++;
        if (lastTimeHashId != 0) {
            timeHash[lastTimeHashId] += time - lastTime;
        }
        lastTimeHashId = id;
        lastTime = time;
    };
...
var baseNode=(function(){
    ...
    function triggerTimer(id) {
        ...
        hitHash[id] ++;
        if (lastTimeHashId != 0) {
            timeHash[lastTimeHashId] += time[0]*1000000*1000+time[1];
        }
        lastTimeHashId = id;
        lastTime=process.hrtime();
            ...
            'function timeMe(){'+
             ...
             generated +
              'triggerTimer("");'+
              'return { times : timeHash, hits : hitHash,statementMap : __conv__.statementMap };' +
            '}'
        },
        /**
...
/*
                incrStatementCount = astgen.statement(
                    astgen.postIncrement(
                        astgen.subscript(
                            astgen.dot(astgen.variable('hello'), astgen.variable('shoes')),
...
*/
                this.splice(astgen.shoes(sName), node, walker);
            }
        },
