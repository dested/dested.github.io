...
VisibilityPolygon.hop = function(cur, next, polygons, position) {
	...
	for (var i = 0; i < polygons.length; ++i) {
		...
		for (var j = 0; j < polygons[i].length; ++j) {
			...
			var a3 = VisibilityPolygon.angle2(polygons[i][j], polygons[i][k], position);
			if (a3 <= 0 || a3 >= 180) continue;
			var a4 = VisibilityPolygon.angle2(polygons[next[0]][next[1]], cur, position);
			if (a4 <= 0 || a4 >= 180 || a4 < a3) {
				if (a3 > best_a) {
					best_a = a3;
					next[0] = i;
					next[1] = j;
