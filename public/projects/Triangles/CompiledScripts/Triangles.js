	...
	$dropTriangles: function() {
		...
		while (bad) {
			...
			for (var y = this.$boardHeight - 1; y >= 0; y--) {
				...
				for (var x = 0; x < this.$boardWidth; x++) {
					...
					if (ss.isNullOrUndefined(current.color) && current.transitioning === 0) {
						...
						if (y === 0 && !current.pointUp) {
							...
							if ($Triangles_TriangleGame.$cascade) {
								return;
							}
							continue;
						}
				...
				if (poppedThisRow && $Triangles_TriangleGame.$rowcascade) {
					return;
				}
			}
...
$Triangles_TriangleGame.$cascade = false;
$Triangles_TriangleGame.$rowcascade = true;
